-- CONSOLIDATED MIGRATION: Fixes all booking/profile schema issues
-- This replaces the split 004_fix_bookings_and_profiles.sql and 004_fix_bookings_and_profiles_clean.sql

-- 1. Drop old status constraint
ALTER TABLE public.bookings DROP CONSTRAINT IF EXISTS bookings_status_check;

-- 2. Rename columns to match TypeScript interface
ALTER TABLE public.bookings RENAME COLUMN IF EXISTS seats_booked TO seats;
ALTER TABLE public.bookings RENAME COLUMN IF EXISTS total_fare TO fare_total;
...
-- 4. Add correct status constraint with all valid statuses
